...
class EngineUtils {
    ...
    static String addBodyLength(String body) {
        return Tag.BODY_LENGTH.value() + TAGVALUE_SEPARATOR + body.length() + SEPARATOR + body;
    }

    static String addChecksum(String body) {
        String checksum = StringUtils.leftPad(calculateChecksum(body).toString(), 3, '0');
        return body + SEPARATOR + Tag.CHECKSUM.value()
                + TAGVALUE_SEPARATOR + checksum;
    }

    static Map.Entry<Tag, String> extractTagValue(String tagValue) {
        String[] parts = StringUtils.strip(tagValue, SEPARATOR).split(TAGVALUE_SEPARATOR, 2);
        if (parts.length != 2)
